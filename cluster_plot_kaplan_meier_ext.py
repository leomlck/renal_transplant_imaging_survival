...
save_folder = os.path.join(main_path, save_folder, args.mod, args.pretraining, 'ext')
...
for i_setup, setup in enumerate(setups):
    ...
    for j_el, (el, keep) in enumerate(zip(['early', 'late'], ['first', 'last'])):
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(main_path, save_folder, 'kaplan_meier_imaging_{}_{}'.format(el, i_setup)))
