...
model_folder = 'RandomSurvivalForest_el' if args.survival_model=='rsf' else 'CoxPHSurvivalAnalysis_el'

save_folder = 'results_survival_analysis_{}_ext'.format(args.event)
save_folder = os.path.join(main_path, save_folder, args.mod, args.pretraining)

with open(os.path.join(main_path, save_folder, args.exam, model_folder, 'ordered_feats_{}.json'.format(args.survival_model))) as fp:
    selected_feats = json.load(fp)
selected_feats = selected_feats['best_model_ordered_feats']
save_folder = os.path.join(save_folder, 'ext')
...
setups = []
setups.append(df_features_ext)
#setups.append(df_features_ext[(df_features_ext.exam >= 0) & (df_features_ext.exam < 23)])
#setups.append(df_features_ext[(df_features_ext.exam >= 23) & (df_features_ext.exam < 61)])
#setups.append(df_features_ext[(df_features_ext.exam >= 61) & (df_features_ext.exam < 228)])
#setups.append(df_features_ext[(df_features_ext.exam >= 228)])

for i_setup, setup in enumerate(setups):
    # clustering features ext
    n_clusters = 2
    ...
    Xt = setup.merge(df_ext[['patient', 'event', 'duration']], how='left', on='patient')
    for j_el, (el, keep) in enumerate(zip(['early', 'late'], ['first', 'last'])):
        X = Xt.sort_values(by='exam').drop_duplicates(subset='patient', keep=keep)
        #X = X.drop(columns=['patient', 'exam'])
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(main_path, save_folder, 'kaplan_meier_imaging_{}_{}_selected_{}_{}'.format(el, i_setup, model_folder, args.exam)))
